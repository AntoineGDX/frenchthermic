...
const SERVICES = [
  { icon: 'Wind', tag: 'Phare', title: 'Clim sans unité extérieure', body: 'Rafraîchir et chauffer sans groupe en façade — idéal en copropriété, centre-ville ou bâtiment classé.', items: ['Sans modification de façade', 'Silencieux', 'Réversible chaud / froid'] },
  { icon: 'Gauge', tag: null, title: 'Audit énergétique & DPE', body: 'Diagnostic complet, classe énergétique et plan de travaux chiffré avec les aides mobilisables.', items: ['DPE réglementaire', 'Scénarios de rénovation', 'MaPrimeRénov’ & CEE'] },
  { icon: 'Compass', tag: null, title: 'Conseil technique', body: 'Accompagnement indépendant, du dimensionnement à la réception du chantier.', items: ['Étude thermique', 'Choix des équipements', 'Maîtrise d’œuvre'] },
];
        ...
        {SERVICES.map((s) => (
          <SCard key={s.title} accent={!!s.tag} hoverable padding="lg" style={{ display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ marginTop: 'auto' }}>
              <SBtn variant="ghost" onClick={() => onNavigate('service')} iconRight={<span data-ico="ArrowRight" data-ico-size="16" />} style={{ padding: '0 2px', height: 'auto' }}>En savoir plus</SBtn>
            </div>
          </SCard>
        ))}
      </div>
